...
vec2 cubemapFaceUV(vec3 d, int face) {
    vec3 a = abs(d);
    if (face == 0) return vec2(-d.z, -d.y) / max(a.x, 1e-8);
    if (face == 1) return vec2(d.z, -d.y) / max(a.x, 1e-8);
    if (face == 2) return vec2(d.x,  d.z) / max(a.y, 1e-8);
    if (face == 3) return vec2(d.x, -d.z) / max(a.y, 1e-8);
    if (face == 4) return vec2(d.x, -d.y) / max(a.z, 1e-8);
    return             vec2(-d.x, -d.y) / max(a.z, 1e-8);
}
vec3 faceDebugColor(int face) {
    if (face == 0) return vec3(1.0, 0.2, 0.2);
    if (face == 1) return vec3(0.2, 1.0, 0.2);
    if (face == 2) return vec3(0.2, 0.4, 1.0);
    if (face == 3) return vec3(1.0, 1.0, 0.2);
    if (face == 4) return vec3(1.0, 0.2, 1.0);
...
vec3 nightStars(vec3 dir, float sunElevation) {
    if (sunElevation > 0.1) return vec3(0.0);  // Not visible during day
