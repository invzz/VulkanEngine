  ...
  void ResourceManager::applyLoadedLightmapsToScene(engine::Scene& scene)
  {
    ...
    for (auto entity : view)
    {
      ...
      if (auto tex = it->second.lock())
      {
        ...
        {
          // No material to attach the texture to — set the LightmapComponent's textureIndex
          // so the renderer can sample the lightmap directly from the bindless texture array.
          lmComp.textureIndex = static_cast<int>(tex->getGlobalIndex());
          std::cout << "ResourceManager: object '" << objName << "' has no PBRMaterial; set LightmapComponent.textureIndex = " << lmComp.textureIndex << " for lightmap '" << lmComp.lightmapId
                    << "'\n";
        }
      }
      else
