...
Texture::Texture(Device &device, const std::string &filepath, bool srgb,
  ...
  stbi_uc *pixels = stbi_load(filepath.c_str(), &width_, &height_, &texChannels,
                              STBI_rgb_alpha);
  ...
  mipLevels_ =
      static_cast<uint32_t>(std::floor(std::log2(std::max(width_, height_)))) +
      1;

  // Create staging buffer
  Buffer stagingBuffer{device_, 1, static_cast<uint32_t>(imageSize),
  ...
  // Create Vulkan image
  createImage(width_, height_, mipLevels_, format, VK_IMAGE_TILING_OPTIMAL,
              VK_IMAGE_USAGE_TRANSFER_SRC_BIT |
                  VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
              VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

  // Transition image layout and copy buffer to image
  transitionImageLayout(image_, format, VK_IMAGE_LAYOUT_UNDEFINED,
                        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels_);
  copyBufferToImage(stagingBuffer.getBuffer(), image_,
  ...
  std::cout << "[" << GREEN << "Texture" << RESET << "] Loaded: " << filepath
            << " (" << width_ << "x" << height_ << ", " << mipLevels_
            << " mips)" << std::endl;
}
...
Texture::Texture(Device &device, const unsigned char *pixels, int width,
  ...
  copyBufferToImage(stagingBuffer.getBuffer(), image_,
                    static_cast<uint32_t>(width_),
                    static_cast<uint32_t>(height_));
  transitionImageLayout(image_, format, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
std::shared_ptr<Texture> Texture::createWhiteTexture(Device &device) {
  ...
  return std::shared_ptr<Texture>(
      new Texture(device, whitePixel, 1, 1, VK_FORMAT_R8G8B8A8_UNORM));
}
...
std::shared_ptr<Texture> Texture::createNormalTexture(Device &device) {
  ...
  return std::shared_ptr<Texture>(
      new Texture(device, normalPixel, 1, 1, VK_FORMAT_R8G8B8A8_UNORM));
}

void Texture::createImage(int width, int height, uint32_t mipLevels,
  ...
  if (vkCreateImage(device_.device(), &imageInfo, nullptr, &image_) !=
      VK_SUCCESS) {
  ...
  allocInfo.memoryTypeIndex = device_.memory().findMemoryType(
      memRequirements.memoryTypeBits, properties);

  if (vkAllocateMemory(device_.device(), &allocInfo, nullptr, &imageMemory_) !=
      VK_SUCCESS) {
...
void Texture::createImageView(VkFormat format) {
  ...
  if (vkCreateImageView(device_.device(), &viewInfo, nullptr, &imageView_) !=
      VK_SUCCESS) {
...
void Texture::createSampler() {
  ...
  if (vkCreateSampler(device_.device(), &samplerInfo, nullptr, &sampler_) !=
      VK_SUCCESS) {
...
void Texture::generateMipmaps(VkImage image, VkFormat format, int32_t width,
                              int32_t height, uint32_t mipLevels) {
  // Check if image format supports linear blitting
  VkFormatProperties formatProperties;
  vkGetPhysicalDeviceFormatProperties(device_.getPhysicalDevice(), format,
                                      &formatProperties);
