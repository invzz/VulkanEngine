...
Model::Model(Device &device, const Builder &builder)
    : device{device}, materials_{builder.materials},
      subMeshes_{builder.subMeshes}, animations_{builder.animations},
      nodes_{builder.nodes}, morphTargetSets_{builder.morphTargetSets},
...
std::unique_ptr<Model> Model::createModelFromFile(Device &device,
                                                  ...
                                                  bool flipZ) {
  std::cout << "[" << GREEN << "Model" << RESET
            << "]: Loading model from file: " << filepath << std::endl;
  Builder builder;
  builder.loadModelFromFile(filepath, flipX, flipY, flipZ);
  std::cout << "[" << GREEN << "Model" << RESET << "]: " << filepath << " with "
            << builder.vertices.size() << " vertices " << std::endl;
...
void Model::drawSubMesh(VkCommandBuffer commandBuffer,
                        size_t subMeshIndex) const {
...
void Model::bindAlternateVertexBuffer(VkCommandBuffer commandBuffer,
                                      VkBuffer alternateVertexBuffer) const {
...
void Model::createIndexBuffers(const std::vector<uint32_t> &indices) {
  ...
  device.memory().copyBufferImmediate(
...
std::unique_ptr<Model> Model::createModelFromGLTF(Device &device,
                                                  const std::string &filepath,
                                                  bool flipX, bool flipY,
                                                  bool flipZ) {
  std::cout << "[" << GREEN << "Model" << RESET
            << "]: Loading glTF model from file: " << filepath << std::endl;
  Builder builder;
...
void Model::Builder::loadModelFromGLTF(const std::string &filepath, bool flipX,
...
void Model::generateMeshlets(const std::vector<Vertex> &vertices,
  ...
  for (auto &subMesh : subMeshes_) {
    size_t max_meshlets = meshopt_buildMeshletsBound(
        subMesh.indexCount, max_vertices, max_triangles);

    std::vector<meshopt_Meshlet> localMeshlets(max_meshlets);
    std::vector<unsigned int> local_meshlet_vertices(max_meshlets *
                                                     max_vertices);
    std::vector<unsigned char> local_meshlet_triangles(max_meshlets *
                                                       max_triangles * 3);

    size_t meshlet_count = meshopt_buildMeshlets(
        localMeshlets.data(), local_meshlet_vertices.data(),
        local_meshlet_triangles.data(), &indices[subMesh.indexOffset],
        subMesh.indexCount, &vertices[0].position.x, vertices.size(),
    ...
    local_meshlet_triangles.resize(last.triangle_offset +
    ...
    uint32_t vertexOffsetBase =
        static_cast<uint32_t>(all_meshlet_vertices.size());
    uint32_t triangleOffsetBase =
        static_cast<uint32_t>(all_meshlet_triangles.size());

    // Append data
    all_meshlet_vertices.insert(all_meshlet_vertices.end(),
                                local_meshlet_vertices.begin(),
                                local_meshlet_vertices.end());
    all_meshlet_triangles.insert(all_meshlet_triangles.end(),
                                 local_meshlet_triangles.begin(),
                                 local_meshlet_triangles.end());

    for (auto i = 0; i < meshlet_count; ++i) {
      auto &m = localMeshlets[i];
      meshopt_Bounds bounds = meshopt_computeMeshletBounds(
          &local_meshlet_vertices[m.vertex_offset],
          &local_meshlet_triangles[m.triangle_offset], m.triangle_count,
  ...
  // Meshlet Buffer
  {
    ...
    Buffer stagingBuffer{device, sizeof(Meshlet),
    ...
    meshletBuffer = std::make_unique<Buffer>(
        device, sizeof(Meshlet), static_cast<uint32_t>(meshlets.size()),
        VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT |
            VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

    device.memory().copyBufferImmediate(
        stagingBuffer.getBuffer(), meshletBuffer->getBuffer(), bufferSize,
  ...
  // Meshlet Vertices Buffer
  {
    ...
    Buffer stagingBuffer{device, sizeof(unsigned int),
                         static_cast<uint32_t>(all_meshlet_vertices.size()),
                         VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                         VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                             VK_MEMORY_PROPERTY_HOST_COHERENT_BIT};
    ...
    meshletVerticesBuffer = std::make_unique<Buffer>(
        device, sizeof(unsigned int),
        static_cast<uint32_t>(all_meshlet_vertices.size()),
        VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT |
            VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    ...
    device.memory().copyBufferImmediate(
        stagingBuffer.getBuffer(), meshletVerticesBuffer->getBuffer(),
        bufferSize, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
  ...
  // Meshlet Triangles Buffer
  {
    ...
    Buffer stagingBuffer{device, sizeof(unsigned char),
                         static_cast<uint32_t>(all_meshlet_triangles.size()),
                         VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                         VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                             VK_MEMORY_PROPERTY_HOST_COHERENT_BIT};
    ...
    meshletTrianglesBuffer = std::make_unique<Buffer>(
        device, sizeof(unsigned char),
        static_cast<uint32_t>(all_meshlet_triangles.size()),
        VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT |
            VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    ...
    device.memory().copyBufferImmediate(
        stagingBuffer.getBuffer(), meshletTrianglesBuffer->getBuffer(),
        bufferSize, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
        VK_ACCESS_SHADER_READ_BIT);
  }

  std::cout << "[" << GREEN << "Model" << RESET << "] Generated "
            << meshlets.size() << " meshlets." << std::endl;
}
