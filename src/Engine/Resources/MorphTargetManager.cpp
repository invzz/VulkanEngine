...
void MorphTargetManager::initializeModel(std::shared_ptr<Model> model) {
  ...
  std::cout << "[" << GREEN << "MorphTargetManager" << RESET
            << "] Initialized model with " << data.morphTargetCount
            << " morph targets, " << data.vertexCount << " vertices"
            << std::endl;
}

void MorphTargetManager::createMorphBuffers(const Model &model,
                                            ModelMorphData &data) {
  ...
  for (const auto &target : morphSet.targets) {
    for (size_t i = 0; i < data.vertexCount; i++) {
      ...
      uint32_t posIdx = i;
      if (!morphSet.positionIndices.empty() &&
          i < morphSet.positionIndices.size()) {
    ...
    if (&target == &morphSet.targets[0]) {
      ...
      for (size_t i = 0; i < std::min(static_cast<size_t>(6),
                                      morphSet.positionIndices.size());
           i++) {
  ...
  data.morphDeltaBuffer = std::make_unique<Buffer>(
      device_, sizeof(MorphDelta), static_cast<uint32_t>(deltas.size()),
      VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

  // Upload delta data
  Buffer stagingBuffer{device_, sizeof(MorphDelta),
  ...
  stagingBuffer.map();
  stagingBuffer.writeToBuffer(deltas.data(),
                              sizeof(MorphDelta) * deltas.size());
  stagingBuffer.unmap();

  device_.memory().copyBufferImmediate(
      stagingBuffer.getBuffer(), data.morphDeltaBuffer->getBuffer(),
      sizeof(MorphDelta) * deltas.size(), VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
      VK_ACCESS_SHADER_READ_BIT);

  // Create weights buffer (will be updated each frame)
  data.weightsBuffer = std::make_unique<Buffer>(
      device_, sizeof(float), static_cast<uint32_t>(weightsCount),
  ...
  if (!morphSet.weights.empty()) {
    for (size_t i = 0; i < std::min(weightsCount, morphSet.weights.size());
         i++) {
      initialWeights[i] = morphSet.weights[i];
    }
  }
  data.weightsBuffer->writeToBuffer(initialWeights.data(),
                                    sizeof(float) * weightsCount);

  // Create blended output buffer (will store computed vertices)
  data.blendedBuffer = std::make_unique<Buffer>(
      device_, sizeof(Model::Vertex), static_cast<uint32_t>(data.vertexCount),
      VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT |
          VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
}

void MorphTargetManager::updateAndBlend(VkCommandBuffer commandBuffer,
  ...
  for (const auto &node : nodes) {
    if (!node.morphWeights.empty()) {
      ...
      for (size_t i = 0;
           i < std::min(currentWeights.size(), node.morphWeights.size()); i++) {
  ...
  data.weightsBuffer->writeToBuffer(currentWeights.data(),
                                    sizeof(float) * currentWeights.size());
  ...
  if (!printedOnce) {
    std::cout << "[MorphTargetManager] Compute dispatch: offset="
              << pushConstants.vertexOffset
              << " count=" << pushConstants.vertexCount
              << " morphTargets=" << pushConstants.morphTargetCount
              << std::endl;
    printedOnce = true;
  }

  // Dispatch compute shader and cache descriptor set
  data.descriptorSet = compute_->blend(
      commandBuffer, data.descriptorSet, model->getVertexBuffer(),
      data.morphDeltaBuffer->getBuffer(), data.weightsBuffer->getBuffer(),
      data.blendedBuffer->getBuffer(), pushConstants);
