...
void TextureManager::createDescriptorSet() {
  if (!descriptorPool->allocateDescriptor(
          descriptorSetLayout->getDescriptorSetLayout(), descriptorSet)) {
    throw std::runtime_error(
        "failed to allocate global texture descriptor set!");
  }
}
...
void TextureManager::updateDescriptorSet(uint32_t index,
