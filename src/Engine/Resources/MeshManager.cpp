...
uint32_t MeshManager::registerModel(const Model *model) {
  ...
  std::cout << "[" << GREEN << "MeshManager" << RESET
            << "] Registered model with ID " << id
            << " (VA: " << info.vertexBufferAddress
...
void MeshManager::updateBuffer() {
  ...
  Buffer stagingBuffer{device, sizeof(MeshBuffers),
  ...
  meshBuffer = std::make_unique<Buffer>(
      device, sizeof(MeshBuffers), static_cast<uint32_t>(meshInfos.size()),
      VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

  device.memory().copyBufferImmediate(
      stagingBuffer.getBuffer(), meshBuffer->getBuffer(), bufferSize,
