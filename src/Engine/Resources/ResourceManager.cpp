...
ResourceManager::~ResourceManager() { shutdownThreadPool(); }

std::string ResourceManager::makeTextureKey(const std::string &path,
                                            bool srgb) const {
...
std::string ResourceManager::makeModelKey(const std::string &path,
                                          bool enableTextures,
                                          bool loadMaterials,
                                          bool enableMorphTargets) const {
  // Include loading flags in key since same model can be loaded with different
  // settings
  std::ostringstream oss;
  oss << path << "|tex=" << enableTextures << "|mat=" << loadMaterials
      << "|morph=" << enableMorphTargets;
  return oss.str();
}

std::shared_ptr<Texture>
ResourceManager::loadTexture(const std::string &path, bool srgb, bool flipY,
  ...
  if (it != textureCache_.end()) {
    ...
    if (auto cachedTexture = it->second.lock()) {
      ...
      textureAccessOrder_.erase(
          std::remove_if(
              textureAccessOrder_.begin(), textureAccessOrder_.end(),
              [&key](const ResourceInfo &info) { return info.key == key; }),
...
std::shared_ptr<Model> ResourceManager::loadModel(const std::string &path,
                                                  ...
                                                  ResourcePriority priority) {
  std::string key =
      makeModelKey(path, enableTextures, loadMaterials, enableMorphTargets);
  ...
  if (it != modelCache_.end()) {
    ...
    if (auto cachedModel = it->second.lock()) {
      ...
      // Remove from access tracking
      modelAccessOrder_.erase(std::remove_if(modelAccessOrder_.begin(),
  ...
  // Load new model
  auto model = std::shared_ptr<Model>(Model::createModelFromFile(
      device_, path, enableTextures, loadMaterials, enableMorphTargets));
...
std::shared_ptr<Texture> ResourceManager::loadTextureFromMemory(
    const unsigned char *data, size_t dataSize, const std::string &debugName,
    bool srgb, ResourcePriority priority) {
  ...
  cacheKey = "embedded:" + contentHash + "|" + debugName +
             (srgb ? "|srgb" : "|linear");
  ...
  // Clean up textures
  {
    ...
    for (auto it = textureCache_.begin(); it != textureCache_.end();) {
      if (it->second.expired()) {
        // Remove from access tracking
        textureAccessOrder_.erase(
            std::remove_if(textureAccessOrder_.begin(),
                           textureAccessOrder_.end(),
...
bool ResourceManager::isTextureCached(const std::string &path) const {
  ...
  bool srgbCached =
      (srgbIt != textureCache_.end() && !srgbIt->second.expired());
  bool linearCached =
      (linearIt != textureCache_.end() && !linearIt->second.expired());
...
void ResourceManager::updateTextureAccess(const std::string &key,
                                          size_t memorySize,
                                          ResourcePriority priority) {
  // Remove existing entry if present
  textureAccessOrder_.erase(std::remove_if(textureAccessOrder_.begin(),
                                           textureAccessOrder_.end(),
                                           [&key](const ResourceInfo &info) {
                                             return info.key == key;
                                           }),
...
void ResourceManager::updateModelAccess(const std::string &key,
                                        size_t memorySize,
                                        ResourcePriority priority) {
  // Remove existing entry if present
  modelAccessOrder_.erase(std::remove_if(modelAccessOrder_.begin(),
...
void ResourceManager::evictLRUTextures() {
  ...
  // first)
  std::sort(textureAccessOrder_.begin(), textureAccessOrder_.end(),
  ...
  size_t evictIndex = 0;
  while (evictIndex < textureAccessOrder_.size() &&
         textureAccessOrder_[evictIndex].priority ==
...
void ResourceManager::evictLRUModels() {
  ...
  // Sort by priority first (low priority first), then by access time (oldest
  // first)
  std::sort(modelAccessOrder_.begin(), modelAccessOrder_.end(),
            [](const ResourceInfo &a, const ResourceInfo &b) {
              if (a.priority != b.priority)
                return a.priority < b.priority; // Lower priority evicted first
              return a.lastAccessTime < b.lastAccessTime; // Then oldest
  ...
  // Skip CRITICAL priority resources
  size_t evictIndex = 0;
  while (evictIndex < modelAccessOrder_.size() &&
         modelAccessOrder_[evictIndex].priority == ResourcePriority::CRITICAL) {
    ++evictIndex;
  }
...
std::string ResourceManager::computeContentHash(const unsigned char *data,
...
void ResourceManager::workerThreadLoop() {
  while (true) {
...

    {
      ...
      taskQueueCV_.wait(
          lock, [this] { return shutdownThreadPool_ || !taskQueue_.empty(); });
...
ResourceManager::loadTextureAsync(const std::string &path, bool srgb,
                                  ResourcePriority priority) {
...
ResourceManager::loadModelAsync(const std::string &path, bool enableTextures,
                                bool loadMaterials, bool enableMorphTargets,
                                ResourcePriority priority) {
  // Check if already cached (fast path)
  std::string key =
      makeModelKey(path, enableTextures, loadMaterials, enableMorphTargets);
  ...
  {
    std::lock_guard<std::mutex> lock(taskQueueMutex_);
    taskQueue_.push([this, path, enableTextures, loadMaterials,
                     enableMorphTargets, priority, promise]() {
      try {
        // Load model synchronously on worker thread
        auto model = loadModel(path, enableTextures, loadMaterials,
                               enableMorphTargets, priority);
