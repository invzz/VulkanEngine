...
CubeShadowMap::CubeShadowMap(Device &device, uint32_t size)
...
void CubeShadowMap::createDepthResources() {
  ...
  if (vkCreateImage(device_.device(), &imageInfo, nullptr, &depthImage_) !=
      VK_SUCCESS) {
  ...
  allocInfo.memoryTypeIndex = device_.getMemory().findMemoryType(
      memRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

  if (vkAllocateMemory(device_.device(), &allocInfo, nullptr,
                       &depthImageMemory_) != VK_SUCCESS) {
  ...
  if (vkCreateImageView(device_.device(), &cubeViewInfo, nullptr,
                        &cubeImageView_) != VK_SUCCESS) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (vkCreateImageView(device_.device(), &faceViewInfo, nullptr,
                          &faceImageViews_[i]) != VK_SUCCESS) {
      throw std::runtime_error(
          "Failed to create cube shadow map face image view");
    }
  }
...
glm::mat4 CubeShadowMap::getFaceViewMatrix(const glm::vec3 &lightPos,
                                           int face) {
...
void CubeShadowMap::transitionToAttachmentLayout(
...
void CubeShadowMap::transitionToShaderReadLayout(
    VkCommandBuffer commandBuffer) {
