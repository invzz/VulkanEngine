...
uint32_t HZBGenerator::calculateMipLevels(VkExtent2D extent) {
  ...
  return static_cast<uint32_t>(
             std::floor(std::log2(static_cast<double>(maxDim)))) +
         1;
}
...
void HZBGenerator::createPipelineIfNeeded() {
  ...
  const std::string shaderPath =
      std::string(SHADER_PATH) + "/hiz_generate.comp.spv";
#else
  const std::string shaderPath =
      "assets/shaders/compiled/hiz_generate.comp.spv";
#endif
  ...
  if (vkCreateShaderModule(device_.device(), &shaderCreateInfo, nullptr,
...
void HZBGenerator::recreateDescriptors(FrameBuffer &frameBuffer,
                                       VkExtent2D extent,
  ...
  if (hzbDescriptorPool_ != VK_NULL_HANDLE && mipLevels_ == mipLevels &&
  ...
  for (uint32_t frame = 0; frame < framesInFlight_; frame++) {
    ...
    const VkResult allocResult = vkAllocateDescriptorSets(
        device_.device(), &allocInfo, hzbDescriptorSets_[frame].data());
    if (allocResult != VK_SUCCESS) {
      throw std::runtime_error(
          "failed to allocate HZB descriptor sets! Error: " +
    ...
    for (uint32_t mip = 0; mip < setsPerFrame; mip++) {
      ...
      if (mip == 0) {
        ...
        inputView = frameBuffer.getHzbMipImageView(static_cast<int>(frame),
                                                   static_cast<int>(mip - 1));
        outputView = frameBuffer.getHzbMipImageView(static_cast<int>(frame),
                                                    static_cast<int>(mip));
...
void HZBGenerator::generate(VkCommandBuffer commandBuffer,
  ...
  for (uint32_t mip = 0; mip < mipLevels_; mip++) {
    const int32_t currentWidth =
        std::max(1, width >> static_cast<int32_t>(mip));
    const int32_t currentHeight =
        std::max(1, height >> static_cast<int32_t>(mip));

    const uint32_t mode = (mip == 0) ? 0u : 1u;
    vkCmdPushConstants(commandBuffer, hzbPipelineLayout_,
                       VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(uint32_t), &mode);

    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE,
                            hzbPipelineLayout_, 0, 1,
                            &hzbDescriptorSets_[frameIndex][mip], 0, nullptr);

    vkCmdDispatch(commandBuffer, (currentWidth + 31) / 32,
                  (currentHeight + 31) / 32, 1);
