...
void Device::pickPhysicalDevice() {
  ...
  for (const auto &device : devices) {
    ...
    if (bestDevice == VK_NULL_HANDLE ||
        ...
        props.deviceType == best.deviceType &&
            props.limits.maxImageDimension2D >
                best.limits.maxImageDimension2D) {
      bestDevice = device;
      best = props;
    }
  }
...
void Device::createLogicalDevice() {
  ...
  static_assert(sizeof(PFN_vkGetPhysicalDeviceFeatures2KHR) ==
  ...
  vkCmdDrawMeshTasksEXT = (PFN_vkCmdDrawMeshTasksEXT)vkGetDeviceProcAddr(
      device_, "vkCmdDrawMeshTasksEXT");
