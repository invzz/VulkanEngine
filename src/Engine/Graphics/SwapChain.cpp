...
VkResult SwapChain::submitCommandBuffers(const VkCommandBuffer *buffers,
  ...
  VkResult submitResult = vkQueueSubmit(device.graphicsQueue(), 1, &submitInfo,
                                        inFlightFences[currentFrame]);
  if (submitResult != VK_SUCCESS) {
    throw CommandBufferSubmissionException(
        "failed to submit draw command buffer! Error: " +
...
void SwapChain::createSwapChain() {
...
#endif
  if (VkResult createResult = vkCreateSwapchainKHR(device.device(), &createInfo,
                                                   nullptr, &swapChain);
  ...
  if (VkResult getImagesResult = vkGetSwapchainImagesKHR(
          device.device(), swapChain, &actualImageCount, nullptr);
  ...
  if (VkResult getImagesResult =
          vkGetSwapchainImagesKHR(device.device(), swapChain, &actualImageCount,
...
void SwapChain::createSyncObjects() {
  ...
  for (auto &fence : inFlightFences) {
    if (vkCreateFence(device.device(), &fenceInfo, nullptr, &fence) !=
        VK_SUCCESS) {
...
VkPresentModeKHR SwapChain::chooseSwapPresentMode(
  ...
  for (auto preferred : preferredModes) {
    if (std::find(availablePresentModes.begin(), availablePresentModes.end(),
                  preferred) != availablePresentModes.end()) {
