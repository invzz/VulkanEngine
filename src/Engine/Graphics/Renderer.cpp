...
Renderer::Renderer(Window &window, Device &device)
    : window{window}, device{device}, hzbGenerator{device} {
  recreateSwapChain();
...
void Renderer::recreateSwapChain() {
  ...
  if (swapChain == nullptr) {
    ...
    if (!oldSwapChain->compareSwapFormats(*swapChain)) {
      throw SwapChainCreationException(
...
VkCommandBuffer Renderer::beginFrame() {
  ...
  if (auto beginCommandBufferResult =
    ...
    throw CommandBufferRecordingException(
...
void Renderer::createOffscreenResources() {
  offscreenFrameBuffer =
      std::make_unique<FrameBuffer>(device, swapChain->getSwapChainExtent(),
                                    SwapChain::maxFramesInFlight(), true);
}
...
void Renderer::createHZBPipeline() {
  if (!offscreenFrameBuffer)
    return;
  hzbGenerator.recreateDescriptors(
      *offscreenFrameBuffer, swapChain->getSwapChainExtent(),
...
void Renderer::generateDepthPyramid(VkCommandBuffer commandBuffer) {
  if (!offscreenFrameBuffer)
    return;
  hzbGenerator.generate(commandBuffer, *offscreenFrameBuffer,
                        swapChain->getSwapChainExtent(), currentFrameIndex);
}
