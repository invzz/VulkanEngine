...
void DescriptorManager::allocatePerFrameDescriptors(Renderer& renderer) {
    ...
    for (const auto& ds : gbufferDescriptorSets_) assert(ds != VK_NULL_HANDLE);
    for (const auto& ds : postProcessDescriptorSets_) assert(ds != VK_NULL_HANDLE);
    for (const auto& ds : deferredShadowDescriptorSets_) assert(ds != VK_NULL_HANDLE);
}
...
void DescriptorManager::updateShadowDescriptors(int frameIndex, ShadowSystem& shadowSystem, Device& device) {
    if (frameIndex < 0 || frameIndex >= static_cast<int>(deferredShadowDescriptorSets_.size())
        || deferredShadowDescriptorSets_[frameIndex] == VK_NULL_HANDLE) {
        return;
    }
