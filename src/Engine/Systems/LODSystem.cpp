...
void LODSystem::update(FrameInfo &frameInfo) {
  ...
  auto view = frameInfo.scene->getRegistry()
                  .view<LODComponent, TransformComponent, ModelComponent>();
  for (auto entity : view) {
    auto [lod, transform, modelComp] =
        view.get<LODComponent, TransformComponent, ModelComponent>(entity);
    if (lod.levels.empty())
      continue;
    ...
    // Actually, usually LOD levels are defined as "start using this LOD at this
    // distance". So: LOD0: 0 LOD1: 10 LOD2: 50
    ...
    // So we want the level with the highest distance that is <= current
    // distance.

    std::shared_ptr<Model> selectedModel = nullptr;

    // Iterate in reverse to find the first one that satisfies distance >=
    // level.distance Assuming levels are sorted by distance ascending. If not
    // sorted, we should sort them or just search. Let's assume they might not
    // be sorted, so we search for the best fit.
    ...
    // If distance is smaller than all LODs (e.g. closest LOD starts at 10, but
    // we are at 5), we should probably use the one with smallest distance
    // (LOD0).
