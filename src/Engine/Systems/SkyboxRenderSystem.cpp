...
SkyboxRenderSystem::SkyboxRenderSystem(Device &device, VkRenderPass renderPass)
    : device_{device} {
...
SkyboxRenderSystem::~SkyboxRenderSystem() {
  ...
  if (proceduralPipelineLayout_ != VK_NULL_HANDLE) {
    vkDestroyPipelineLayout(device_.device(), proceduralPipelineLayout_,
                            nullptr);
  }
...
void SkyboxRenderSystem::createPipelineLayout() {
  ...
  if (vkCreatePipelineLayout(device_.device(), &layoutInfo, nullptr,
                             &proceduralPipelineLayout_) != VK_SUCCESS) {
    throw std::runtime_error(
        "Failed to create procedural skybox pipeline layout");
  }
}
...
void SkyboxRenderSystem::createPipeline(VkRenderPass renderPass) {
  ...
  pipeline_ = std::make_unique<Pipeline>(
      device_, std::string(SHADER_PATH) + R"(skybox.vert.spv)",
      std::string(SHADER_PATH) + R"(skybox.frag.spv)", configInfo);
}
...
void SkyboxRenderSystem::createProceduralPipeline(VkRenderPass renderPass) {
  ...
  proceduralPipeline_ = std::make_unique<Pipeline>(
      device_, std::string(SHADER_PATH) + R"(procedural_sky.vert.spv)",
      std::string(SHADER_PATH) + R"(procedural_sky.frag.spv)", configInfo);
}
...
void SkyboxRenderSystem::render(FrameInfo &frameInfo, Skybox *skybox,
  ...
  if (settings.useProcedural) {
    ...
    vkCmdPushConstants(frameInfo.commandBuffer, proceduralPipelineLayout_,
    ...
    vkCmdBindDescriptorSets(frameInfo.commandBuffer,
                            VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout_,
                            0, // first set
                            1, // set count
                            &descriptorSets_[frameInfo.frameIndex], 0, nullptr);
