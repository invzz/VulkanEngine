...
namespace engine {
  ...
  void SkyboxRenderSystem::createPipeline(VkRenderPass renderPass)
  {
    ...
    pipeline_ = std::make_unique<Pipeline>(device_, std::string(SHADER_PATH) + R"(skybox.vert.spv)", std::string(SHADER_PATH) + R"(skybox.frag.spv)", configInfo);
  }
  ...
  void SkyboxRenderSystem::createProceduralPipeline(VkRenderPass renderPass)
  {
    ...
    proceduralPipeline_ = std::make_unique<Pipeline>(device_, std::string(SHADER_PATH) + R"(procedural_sky.vert.spv)", std::string(SHADER_PATH) + R"(procedural_sky.frag.spv)", configInfo);
  }
  ...
  void SkyboxRenderSystem::render(FrameInfo& frameInfo, Skybox* skybox, const SkyboxSettings& settings)
  {
    ...
    if (settings.useProcedural)
    {
      ...
      vkCmdPushConstants(frameInfo.commandBuffer, proceduralPipelineLayout_, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(SkyboxPushConstants), &push);
