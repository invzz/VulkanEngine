...
void AnimationSystem::updateAnimations(FrameInfo &frameInfo) {
  auto view = frameInfo.scene->getRegistry()
                  .view<AnimationComponent, TransformComponent>();
  for (auto entity : view) {
    auto [anim, transform] =
        view.get<AnimationComponent, TransformComponent>(entity);
    ...
    const auto &animation =
        anim.model->getAnimations()[anim.currentAnimationIndex];
    ...
    if (modelComp.model && modelComp.model->hasMorphTargets()) {
      ...
      if (!morphManager_->isModelInitialized(modelComp.model.get())) {
        try {
          ...
          std::cerr << "[AnimationSystem] ERROR initializing morph for object "
                    << (uint32_t)entity << ": " << e.what() << std::endl;
          continue;
        }
      }
...
void AnimationSystem::updateNodeTransforms(AnimationComponent &animComp,
  ...
  for (const auto &channel : animation.channels) {
    if (channel.targetNode < 0 ||
        channel.targetNode >= static_cast<int>(nodes.size())) {
      continue;
    }
    ...
    case Model::AnimationChannel::WEIGHTS:
      node.morphWeights =
          interpolateMorphWeights(sampler, animComp.currentTime);
      break;
    }
  }
...
void AnimationSystem::computeGlobalTransforms(
    AnimationComponent &animComp, int nodeIndex,
    const glm::mat4 &parentTransform) {
  if (nodeIndex < 0 ||
      nodeIndex >= static_cast<int>(animComp.model->getNodes().size())) {
    return;
  }
  ...
  for (int childIdx : node.children) {
    computeGlobalTransforms(animComp, childIdx,
                            animComp.nodeTransforms[nodeIndex]);
  }
}

glm::vec3
AnimationSystem::interpolateVec3(const Model::AnimationSampler &sampler,
                                 float time) {
  ...
  return glm::mix(sampler.translations[prevIndex],
                  sampler.translations[nextIndex], factor);
}

glm::quat
AnimationSystem::interpolateQuat(const Model::AnimationSampler &sampler,
                                 float time) {
  ...
  return glm::slerp(sampler.rotations[prevIndex], sampler.rotations[nextIndex],
                    factor);
}

std::vector<float>
AnimationSystem::interpolateMorphWeights(const Model::AnimationSampler &sampler,
                                         float time) {
