...
PrefilteredEnvIBL::PrefilteredEnvIBL(Device& device) : device_(device) {}

PrefilteredEnvIBL::~PrefilteredEnvIBL() = default;

void PrefilteredEnvIBL::resetToUninitialized() {
...
VkDescriptorImageInfo PrefilteredEnvIBL::getDescriptorInfo() const {
...
void PrefilteredEnvIBL::adoptLoaded(VkImage image, VkDeviceMemory memory, VkImageView imageView, VkSampler sampler) {
  image_ = image;
  memory_ = memory;
...
void PrefilteredEnvIBL::deferDestroyImageResources() {
...
void PrefilteredEnvIBL::destroyImmediate() {
...
void PrefilteredEnvIBL::createFallback() {
  ...
  imageView_ = ibl_detail::createImageView(device_, image_, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_ASPECT_COLOR_BIT, 1, VK_IMAGE_VIEW_TYPE_CUBE, 0, 6);
...
void PrefilteredEnvIBL::createForSettings(const Settings& settings) {
  uint32_t const mipLevels = static_cast<uint32_t>((std::max)(1, settings.prefilterMipLevels));

  ibl_detail::createImage(device_,
      settings.prefilterSize,
      settings.prefilterSize,
      mipLevels,
      VK_FORMAT_R16G16B16A16_SFLOAT,
      VK_IMAGE_TILING_OPTIMAL,
  ...
  imageView_ = ibl_detail::createImageView(device_, image_, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels, VK_IMAGE_VIEW_TYPE_CUBE, 0, 6);
  ...
  ibl_detail::transitionImageLayout(device_, image_, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, mipLevels, 6);
}

void PrefilteredEnvIBL::ensurePipelineResources() {
  ...
  pushConstantRange.size = sizeof(glm::mat4) + sizeof(int) + sizeof(float) + sizeof(uint32_t);
...
void PrefilteredEnvIBL::generateFromSkybox(Skybox& skybox, const Settings& settings) {
  ...
  int const mipCount = (std::max)(1, settings.prefilterMipLevels);
  for (int mip = 0; mip < mipCount; ++mip) {
    auto const baseSize = static_cast<uint32_t>(settings.prefilterSize);
    ...
    for (int i = 0; i < 6; ++i) {
      ...
      pushBlock.sampleCount = static_cast<uint32_t>(settings.prefilterSampleCount);
  ...
  ibl_detail::transitionImageLayout(device_,
      image_,
      VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
      VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
      static_cast<uint32_t>((std::max)(1, settings.prefilterMipLevels)),
      6);
}
