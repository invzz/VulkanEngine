...
CameraSystem::CameraSystem(Device &device, VkRenderPass renderPass,
...
void CameraSystem::createPipeline(VkRenderPass renderPass) {
  ...
  pipeline = std::make_unique<Pipeline>(
      device, std::string(SHADER_PATH) + "debug_frustum.vert.spv",
      std::string(SHADER_PATH) + "debug_frustum.frag.spv", pipelineConfig);
}
...
void CameraSystem::render(FrameInfo &frameInfo) const {
  ...
  if (registry.valid(frameInfo.selectedEntity) &&
      registry.all_of<CameraComponent, TransformComponent>(
          frameInfo.selectedEntity)) {
    // Don't render the active camera's frustum (it would be weird/invisible)
    if (frameInfo.selectedEntity == frameInfo.cameraEntity)
      return;

    auto [cameraComp, transform] =
        registry.get<CameraComponent, TransformComponent>(
            frameInfo.selectedEntity);

    glm::mat4 modelMatrix = glm::mat4(1.0f);
    modelMatrix = glm::translate(modelMatrix, transform.translation);
    modelMatrix = glm::rotate(modelMatrix, transform.rotation.y,
                              glm::vec3(0.0f, 1.0f, 0.0f));
    modelMatrix = glm::rotate(modelMatrix, transform.rotation.x,
                              glm::vec3(1.0f, 0.0f, 0.0f));
    modelMatrix = glm::rotate(modelMatrix, transform.rotation.z,
    ...
    push.aspectRatio =
        16.0f / 9.0f; // Approximation, or use actual aspect ratio if stored
...
void CameraSystem::update(FrameInfo &frameInfo, float aspectRatio) const {
  ...
  if (registry.valid(frameInfo.cameraEntity)) {
    ...
    if (registry.all_of<CameraComponent, TransformComponent>(
            frameInfo.cameraEntity)) {
      auto &cameraComp = registry.get<CameraComponent>(frameInfo.cameraEntity);
      const auto &transform =
          registry.get<TransformComponent>(frameInfo.cameraEntity);
...
void CameraSystem::updateCamera(CameraComponent &cameraComp,
                                const TransformComponent &transform,
                                float aspectRatio) const {
  // Update projection
  if (!cameraComp.isOrthographic) {
    cameraComp.camera.setPerspectiveProjection(glm::radians(cameraComp.fovY),
                                               aspectRatio, cameraComp.nearZ,
                                               cameraComp.farZ);
  } else {
    ...
    cameraComp.camera.setOrtographicProjection(
        -orthoWidth, orthoWidth, -orthoHeight, orthoHeight, cameraComp.nearZ,
        cameraComp.farZ);
  }
