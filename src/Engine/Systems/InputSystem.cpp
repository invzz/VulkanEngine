...
InputSystem::InputSystem(Keyboard &keyboard, Mouse &mouse, Window &window)
    : keyboard_{keyboard}, mouse_{mouse}, window_{window} {}

void InputSystem::update(FrameInfo &frameInfo) {
  // Handle cursor toggle (ESC key with debouncing)
  bool toggleKeyPressed =
      keyboard_.isKeyPressed(keyboard_.mappings.toggleCursor);
  ...
  entt::entity controllableEntity = frameInfo.selectedEntity != entt::null
                                        ? frameInfo.selectedEntity
                                        : frameInfo.cameraEntity;

  if (frameInfo.scene->getRegistry().valid(controllableEntity)) {
    auto &transform = frameInfo.scene->getRegistry().get<TransformComponent>(
        controllableEntity);
