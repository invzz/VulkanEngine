...
namespace engine {
  ...
  glm::mat4 ShadowSystem::calculateDirectionalLightMatrix(const glm::vec3& lightDirection, const glm::vec3& sceneCenter, float sceneRadius)
  {
    ...
    glm::vec3 const lightPos = sceneCenter - lightDir * sceneRadius * 3.0f;
    ...
    glm::mat4 const lightView = glm::lookAt(lightPos, sceneCenter, up);

    // Orthographic projection that encompasses the scene
    float const orthoSize = sceneRadius * 2.0f;
    float const nearPlane = 0.1f;
    float const farPlane  = sceneRadius * 6.0f;
  ...
  {
    shadowLightCount_           = 0;
    glm::vec3 const sceneCenter = glm::vec3(0.0f);
