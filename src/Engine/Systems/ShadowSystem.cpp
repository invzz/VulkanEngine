...
ShadowSystem::ShadowSystem(Device &device, uint32_t shadowMapSize)
    : device_{device}, shadowMapSize_{shadowMapSize} {
  // Create multiple shadow maps for directional/spot lights
  for (int i = 0; i < MAX_SHADOW_MAPS; i++) {
    shadowMaps_.push_back(
        std::make_unique<ShadowMap>(device, shadowMapSize, shadowMapSize));
    lightSpaceMatrices_[i] = glm::mat4(1.0f);
  }

  // Create cube shadow maps for point lights
  for (int i = 0; i < MAX_CUBE_SHADOW_MAPS; i++) {
    cubeShadowMaps_.push_back(
        std::make_unique<CubeShadowMap>(device, shadowMapSize));
  ...
  std::cout << "[" << GREEN << "ShadowSystem" << RESET << "] Initialized with "
            << MAX_SHADOW_MAPS << " 2D shadow maps and " << MAX_CUBE_SHADOW_MAPS
            << " cube shadow maps (" << shadowMapSize << "x" << shadowMapSize
            << ")" << std::endl;
}
...
void ShadowSystem::createPipeline() {
  ...
  pipeline_ = std::make_unique<Pipeline>(
      device_, std::string(SHADER_PATH) + R"(shadow.vert.spv)",
      std::string(SHADER_PATH) + R"(shadow.frag.spv)", configInfo);
}

glm::mat4
ShadowSystem::calculateDirectionalLightMatrix(const glm::vec3 &lightDirection,
...
glm::mat4 ShadowSystem::calculateSpotLightMatrix(const glm::vec3 &position,
  ...
  float fov =
      glm::radians(outerCutoffDegrees * 2.0f + 5.0f); // Add 5 degree margin
  float nearPlane = 0.1f;
...
void ShadowSystem::renderShadowMaps(FrameInfo &frameInfo, float sceneRadius) {
  ...
  auto dirView = frameInfo.scene->getRegistry()
                     .view<DirectionalLightComponent, TransformComponent>();
  for (auto entity : dirView) {
    if (shadowLightCount_ >= MAX_SHADOW_MAPS)
      break;
    auto [dirLight, transform] =
        dirView.get<DirectionalLightComponent, TransformComponent>(entity);

    glm::vec3 lightDir = transform.getForwardDir();
    lightSpaceMatrices_[shadowLightCount_] =
        calculateDirectionalLightMatrix(lightDir, sceneCenter, sceneRadius);
    renderToShadowMap(frameInfo, *shadowMaps_[shadowLightCount_],
                      lightSpaceMatrices_[shadowLightCount_]);
    shadowLightCount_++;
  ...
  auto spotView = frameInfo.scene->getRegistry()
                      .view<SpotLightComponent, TransformComponent>();
  for (auto entity : spotView) {
    if (shadowLightCount_ >= MAX_SHADOW_MAPS)
      break;
    auto [spotLight, transform] =
        spotView.get<SpotLightComponent, TransformComponent>(entity);
    ...
    lightSpaceMatrices_[shadowLightCount_] = calculateSpotLightMatrix(
        position, direction, outerCutoffDegrees, range);
    renderToShadowMap(frameInfo, *shadowMaps_[shadowLightCount_],
                      lightSpaceMatrices_[shadowLightCount_]);
    shadowLightCount_++;
  }
...
void ShadowSystem::createCubeShadowPipeline() {
  ...
  cubePipeline_ = std::make_unique<Pipeline>(
      device_, std::string(SHADER_PATH) + R"(cube_shadow.vert.spv)",
      std::string(SHADER_PATH) + R"(cube_shadow.frag.spv)", configInfo);
}

void ShadowSystem::renderPointLightShadowMaps(FrameInfo &frameInfo) {
  cubeShadowLightCount_ = 0;

  auto view = frameInfo.scene->getRegistry()
                  .view<PointLightComponent, TransformComponent>();
  for (auto entity : view) {
    if (cubeShadowLightCount_ >= MAX_CUBE_SHADOW_MAPS)
      break;
    auto [pointLight, transform] =
        view.get<PointLightComponent, TransformComponent>(entity);
    ...
    renderToCubeShadowMap(frameInfo, *cubeShadowMaps_[cubeShadowLightCount_],
                          position, range);
...
glm::mat4 ShadowSystem::calculatePointLightMatrix(const glm::vec3 &position,
                                                  int face, float range) {
  float nearPlane = 0.1f;
  glm::mat4 projection =
      glm::perspective(glm::radians(90.0f), 1.0f, nearPlane, range);
...
void ShadowSystem::renderToCubeShadowMap(FrameInfo &frameInfo,
                                         CubeShadowMap &cubeShadowMap,
                                         const glm::vec3 &position,
                                         float range) {
  for (int face = 0; face < 6; face++) {
    glm::mat4 lightSpaceMatrix =
        calculatePointLightMatrix(position, face, range);
    renderToCubeFace(frameInfo, cubeShadowMap, face, lightSpaceMatrix, position,
                     range);
  }
}

void ShadowSystem::renderToCubeFace(FrameInfo &frameInfo,
                                    CubeShadowMap &cubeShadowMap, int face,
                                    const glm::mat4 &lightSpaceMatrix,
  ...
  for (auto entity : view) {
    ...
    vkCmdPushConstants(frameInfo.commandBuffer, cubePipelineLayout_,
                       VK_SHADER_STAGE_VERTEX_BIT |
                           VK_SHADER_STAGE_FRAGMENT_BIT,
                       0, sizeof(CubeShadowPushConstants), &push);
