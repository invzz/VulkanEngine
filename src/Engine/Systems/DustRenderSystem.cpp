...
DustRenderSystem::DustRenderSystem(Device &device, VkRenderPass renderPass)
    : device{device} {
...
void DustRenderSystem::createPipeline(VkRenderPass renderPass) {
  ...
  pipelineConfig.colorBlendAttachment.srcColorBlendFactor =
  ...
  pipeline =
      std::make_unique<Pipeline>(device, vertPath, fragPath, pipelineConfig);
}
...
void DustRenderSystem::createDustBuffer() {
  ...
  for (auto &v : vertices) {
    v.x =
        ...
        20.0f; // Pre-scale to max expected box size to avoid precision issues?
  ...
  Buffer stagingBuffer{device, bufferSize, 1, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
  ...
  vertexBuffer = std::make_unique<Buffer>(device, bufferSize, 1,
...
void DustRenderSystem::render(FrameInfo &frameInfo,
  ...
  push.cameraPosition =
      glm::vec4(frameInfo.camera.getPosition(), settings.boxSize);
  push.params = glm::vec4(
      frameInfo.frameTime * frameInfo.frameIndex *
          0.1f, // Use total time ideally, but frameIndex works for drift
      settings.particleSize, settings.alpha, settings.heightFalloff);
  ...
  vkCmdPushConstants(frameInfo.commandBuffer, pipelineLayout,
                     VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(DustPushConstants),
                     &push);
  ...
  vkCmdDraw(frameInfo.commandBuffer,
            static_cast<uint32_t>(settings.particleCount), 1, 0, 0);
}
