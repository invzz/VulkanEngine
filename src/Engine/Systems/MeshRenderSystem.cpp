...
MeshRenderSystem::MeshRenderSystem(Device &device, VkRenderPass renderPass,
...
MeshRenderSystem::~MeshRenderSystem() {
  ...
  if (shadowDescriptorSetLayout_ != VK_NULL_HANDLE) {
    vkDestroyDescriptorSetLayout(device.device(), shadowDescriptorSetLayout_,
                                 nullptr);
  }
  ...
  if (iblDescriptorSetLayout_ != VK_NULL_HANDLE) {
    vkDestroyDescriptorSetLayout(device.device(), iblDescriptorSetLayout_,
                                 nullptr);
  }
  ...
  if (materialDescriptorSetLayout_ != VK_NULL_HANDLE) {
    vkDestroyDescriptorSetLayout(device.device(), materialDescriptorSetLayout_,
                                 nullptr);
  }
}
...
void MeshRenderSystem::createShadowDescriptorResources() {
  ...
  if (vkCreateDescriptorSetLayout(device.device(), &layoutInfo, nullptr,
                                  &shadowDescriptorSetLayout_) != VK_SUCCESS) {
  ...
  poolSizes[0].descriptorCount = static_cast<uint32_t>(
      SwapChain::maxFramesInFlight() * ShadowSystem::MAX_SHADOW_MAPS);
  poolSizes[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
  poolSizes[1].descriptorCount = static_cast<uint32_t>(
      SwapChain::maxFramesInFlight() * ShadowSystem::MAX_CUBE_SHADOW_MAPS);
  ...
  if (vkCreateDescriptorPool(device.device(), &poolInfo, nullptr,
                             &shadowDescriptorPool_) != VK_SUCCESS) {
    throw std::runtime_error("Failed to create shadow descriptor pool");
  }

  std::vector<VkDescriptorSetLayout> layouts(SwapChain::maxFramesInFlight(),
  ...
  if (vkAllocateDescriptorSets(device.device(), &allocInfo,
                               shadowDescriptorSets_.data()) != VK_SUCCESS) {
...
void MeshRenderSystem::createIBLDescriptorResources() {
  ...
  if (vkCreateDescriptorSetLayout(device.device(), &layoutInfo, nullptr,
                                  &iblDescriptorSetLayout_) != VK_SUCCESS) {
  ...
  poolSizes[0].descriptorCount =
      static_cast<uint32_t>(SwapChain::maxFramesInFlight() * 3);
  ...
  if (vkAllocateDescriptorSets(device.device(), &allocInfo,
                               iblDescriptorSets_.data()) != VK_SUCCESS) {
...
void MeshRenderSystem::createMaterialDescriptorResources() {
  ...
  for (size_t i = 0; i < SwapChain::maxFramesInFlight(); i++) {
    materialBuffers_[i] =
        std::make_unique<Buffer>(device, atomSize,
  ...
  if (vkCreateDescriptorPool(device.device(), &poolInfo, nullptr,
                             &materialDescriptorPool_) != VK_SUCCESS) {
  ...
  if (vkAllocateDescriptorSets(device.device(), &allocInfo,
                               materialDescriptorSets_.data()) != VK_SUCCESS) {
    throw std::runtime_error("Failed to allocate material descriptor sets");
  }

  for (size_t i = 0; i < SwapChain::maxFramesInFlight(); i++) {
    VkDescriptorBufferInfo bufferInfo =
        materialBuffers_[i]->descriptorInfoForIndex(0);
...
void MeshRenderSystem::createPipelineLayout(
  ...
  std::vector<VkDescriptorSetLayout> descriptorSetLayouts{
      globalSetLayout, bindlessSetLayout, shadowDescriptorSetLayout_,
      iblDescriptorSetLayout_, materialDescriptorSetLayout_};
...
void MeshRenderSystem::createPipeline(VkRenderPass renderPass) {
  ...
  pipeline = std::make_unique<Pipeline>(
      device, std::string(SHADER_PATH) + R"(simple_mesh.task.spv)",
      std::string(SHADER_PATH) + R"(simple_mesh.mesh.spv)",
      std::string(SHADER_PATH) + R"(pbr_shader.frag.spv)", pipelineConfig);
  ...
  transparentConfig.colorBlendAttachment.srcColorBlendFactor =
      VK_BLEND_FACTOR_ONE;
  transparentConfig.colorBlendAttachment.dstColorBlendFactor =
      VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA;
  transparentConfig.colorBlendAttachment.colorBlendOp = VK_BLEND_OP_ADD;
  transparentConfig.colorBlendAttachment.srcAlphaBlendFactor =
      VK_BLEND_FACTOR_ONE;
  transparentConfig.colorBlendAttachment.dstAlphaBlendFactor =
      VK_BLEND_FACTOR_ZERO;
  transparentConfig.colorBlendAttachment.alphaBlendOp = VK_BLEND_OP_ADD;

  // Fix pointer to attachment
  transparentConfig.colorBlendInfo.pAttachments =
      &transparentConfig.colorBlendAttachment;
  ...
  transparentPipeline = std::make_unique<Pipeline>(
      device, std::string(SHADER_PATH) + R"(simple_mesh.task.spv)",
      std::string(SHADER_PATH) + R"(simple_mesh.mesh.spv)",
      std::string(SHADER_PATH) + R"(pbr_shader.frag.spv)", transparentConfig);
}
...
void MeshRenderSystem::render(FrameInfo &frameInfo) {
  ...
  vkCmdBindDescriptorSets(frameInfo.commandBuffer,
                          ...
                          &frameInfo.globalTextureSet, 0, nullptr);
  ...
  if (currentShadowSystem_) {
    ...
    std::array<VkDescriptorImageInfo, ShadowSystem::MAX_SHADOW_MAPS>
        shadowInfos{};
    ...
    for (int i = shadowCount; i < ShadowSystem::MAX_SHADOW_MAPS; i++) {
      shadowInfos[i] =
          shadowCount > 0 ? currentShadowSystem_->getShadowMapDescriptorInfo(0)
                          : currentShadowSystem_->getShadowMapDescriptorInfo(0);
    }

    std::array<VkDescriptorImageInfo, ShadowSystem::MAX_CUBE_SHADOW_MAPS>
        cubeShadowInfos{};
    for (int i = 0;
         i < cubeShadowCount && i < ShadowSystem::MAX_CUBE_SHADOW_MAPS; i++) {
      cubeShadowInfos[i] =
          currentShadowSystem_->getCubeShadowMapDescriptorInfo(i);
    }
    ...
    vkCmdBindDescriptorSets(frameInfo.commandBuffer,
                            VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 2,
                            1, &shadowDescriptorSets_[frameInfo.frameIndex], 0,
                            nullptr);
  }
  ...
  if (currentIBLSystem_ && currentIBLSystem_->isGenerated()) {
    VkDescriptorImageInfo irradianceInfo =
        currentIBLSystem_->getIrradianceDescriptorInfo();
    VkDescriptorImageInfo prefilterInfo =
        currentIBLSystem_->getPrefilteredDescriptorInfo();
    VkDescriptorImageInfo brdfInfo =
        currentIBLSystem_->getBRDFLUTDescriptorInfo();
    ...
    vkUpdateDescriptorSets(device.device(),
                           ...
                           descriptorWrites.data(), 0, nullptr);
    vkCmdBindDescriptorSets(frameInfo.commandBuffer,
                            VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 3,
                            1, &iblDescriptorSets_[frameInfo.frameIndex], 0,
                            nullptr);
  }
  ...
  char *mappedData =
      (char *)materialBuffers_[frameInfo.frameIndex]->getMappedMemory();
  uint32_t dynamicOffsetIndex = 0;

  auto view =
      frameInfo.scene->getRegistry().view<ModelComponent, TransformComponent>();
  ...
  auto renderItem = [&](entt::entity entity, const Model::SubMesh &subMesh,
    ...
    push.meshletTrianglesAddress =
        modelComp.model->getMeshletTrianglesAddress();
    ...
    push.screenSize =
        glm::vec2(frameInfo.extent.width, frameInfo.extent.height);
    ...
    float isSelected =
        ((uint32_t)entity == frameInfo.selectedObjectId) ? 1.0f : 0.0f;
    ...
    if (pMaterial) {
      ...
      if (material.specularGlossinessMap) {
        textureFlags |= (1 << 8);
        specularGlossinessIndex =
            material.specularGlossinessMap->getGlobalIndex();
      }
      ...
      if (material.hasClearcoatRoughnessMap()) {
        ...
        clearcoatRoughnessIndex =
            material.clearcoatRoughnessMap->getGlobalIndex();
      }
      ...
      matData.emissiveInfo =
          glm::vec4(material.emissiveColor, material.emissiveStrength);
      matData.specularGlossinessFactor =
          glm::vec4(material.specularFactor, material.glossinessFactor);
      matData.attenuationColorAndDist =
          glm::vec4(material.attenuationColor, material.attenuationDistance);
    ...
    memcpy(mappedData + (dynamicOffsetIndex * atomSize), &matData,
           sizeof(MaterialUniformData));

    uint32_t dynamicOffset =
        static_cast<uint32_t>(dynamicOffsetIndex * atomSize);
    vkCmdBindDescriptorSets(frameInfo.commandBuffer,
                            VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 4,
                            1, &materialDescriptorSets_[frameInfo.frameIndex],
                            1, &dynamicOffset);
    ...
    vkCmdPushConstants(frameInfo.commandBuffer, pipelineLayout,
                       ...
                       0, sizeof(MeshPushConstantData), &push);
  ...
  for (auto entity : view) {
    auto [modelComp, transform] =
        view.get<ModelComponent, TransformComponent>(entity);
    if (!modelComp.model)
      continue;
    ...
    for (const auto &subMesh : subMeshes) {
      ...
      if (auto *mat =
              frameInfo.scene->getRegistry().try_get<PBRMaterial>(entity)) {
        pMaterial = mat;
      } else if (subMesh.materialId >= 0 &&
                 subMesh.materialId < materials.size()) {
      ...
      if (!isTransparent) {
        ...
        transparentItems.push_back(
            {entity, &subMesh, pMaterial, transform.modelTransform(), dist});
      }
    }
  }
