...
void AnimationClip::step(float deltaTime, const Model& model,
    ...
    if (!active || weight <= 0.0f || clipIndex < 0) return;

    // Get the animation data
    const auto& animations = model.getAnimations();
    if (clipIndex >= static_cast<int>(animations.size())) return;
    ...
    for (const auto& channel : channels) {
        ...
        switch (channel.path) {
            case Model::AnimationChannel::TRANSLATION: {
                if (sampler.translations.empty()) continue;
            ...
            case Model::AnimationChannel::ROTATION: {
                if (sampler.rotations.empty()) continue;
            ...
            case Model::AnimationChannel::SCALE: {
                if (sampler.scales.empty()) continue;
