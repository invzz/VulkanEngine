...
namespace engine {
  ...
  void GravityPhysicsSystem::applyBoundaryDamping(GameObject& obj, float dt) const
  {
    ...
    for (int axis = 0; axis < 2; ++axis)
    {
      ...
      {
        const float repelT     = glm::clamp((absCoord - BorderSettings::repulsionStart) / (BorderSettings::clamp - BorderSettings::repulsionStart), 0.f, 1.f);
      ...
      {
        const float t             = glm::clamp((absCoord - BorderSettings::dampingStart) / (BorderSettings::clamp - BorderSettings::dampingStart), 0.f, 1.f);
