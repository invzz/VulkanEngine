...
namespace window_detail {
    ...
    // Choose a target monitor given cursor availability; returns primary monitor as fallback.
    ...
    // Wait for the window position to stabilize or become non-zero. Returns last pos.
...
namespace engine {
    ...
    void Window::initWindow()
    {
        ...
        // Basic GLFW hints: no GL context, resizable, create hidden so we can position before
        // showing.
        ...
        // If we have a target monitor, compute centered position and request it.
        // Note: on Wayland compositors (Hyperland) the compositor may ignore this request.
        if (targetMonitor)
        {
            int mx = 0;
            int my = 0;
