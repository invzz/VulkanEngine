...
namespace engine {
   ...
   * @brief Holds swapchain capabilities, formats, and present modes for a
   * physical device.
   */
  struct SwapChainSupportDetails
  {
  ...
  /**
   * @class Device
   * @brief Manages Vulkan device, queues, command pool, and related
   * resources.
   */
  class Device
  {
  public:
    ...
    /** @brief Returns true if VK_KHR_present_id was enabled on the logical
     * device. */
    bool supportsPresentId() const { return presentIdSupported_; }

    /**
     * @brief Gets swapchain support details for the current physical
     * device.
    ...
    /**
     * @brief Finds graphics and present queue families for the current
     * physical device.
  ...
  private:
    ...
    /**
     * @brief Sets up the Vulkan debug messenger for validation layer
     * output.
     */
    ...
    /**
     * @brief Creates the Vulkan logical device and retrieves
    ...
    /**
     * @brief Checks if a physical device is suitable for engine
     * requirements.
    ...
    /**
     * @brief Gets required Vulkan instance extensions for GLFW and
     * validation layers.
     * @return Vector of required extension names.
     */

    /**
     * @brief Finds graphics and present queue families for a physical
     * device.
     * @param device Vulkan physical device handle.
     * @return QueueFamilyIndices struct with found indices.
     */
    ...
    /**
     * @brief Populates debug messenger creation info for Vulkan validation
     * layers.
    ...
    /**
     * @brief Checks and prints available and required Vulkan instance
     * extensions.
     */
    void hasGflwRequiredInstanceExtensions() const;

    /**
     * @brief Checks if a physical device supports required Vulkan device
     * extensions.
     * @param device Vulkan physical device handle.
     ...
     * @param device Vulkan physical device handle.
     * @return SwapChainSupportDetails struct with capabilities, formats,
     * and present modes.
     */
