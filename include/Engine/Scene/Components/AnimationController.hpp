...
public:
     ...
     */
    void setDefaultBlendMode(AnimationClip::Mode mode) { defaultMode_ = mode; }
    ...
    [[nodiscard]] bool hasActiveClips() const {
        for (const auto& clip : clips_) {
            if (clip.active && clip.weight > 0.0f) return true;
        }
        return false;
    }
    ...
    AnimationClip* getClip(int clipIndex) {
        for (auto& clip : clips_) {
            if (clip.clipIndex == clipIndex) return &clip;
        }
        return nullptr;
    }
     ...
     */
    void clearEvents() { firedEvents_.clear(); }
