    ...
    for (int iter = 0; iter < iterations; ++iter)
    {
        ...
        for (int y = 0; y < height; ++y)
        {
            for (int x = 0; x < width; ++x)
            {
                ...
                int count = 0;
                // 4-neighbors
                if (x > 0) { auto& n = cur[idx(x-1,y)]; if (n.valid) { accum += n.radiance; ++count; } }
                if (x < width-1) { auto& n = cur[idx(x+1,y)]; if (n.valid) { accum += n.radiance; ++count; } }
                if (y > 0) { auto& n = cur[idx(x,y-1)]; if (n.valid) { accum += n.radiance; ++count; } }
                if (y < height-1) { auto& n = cur[idx(x,y+1)]; if (n.valid) { accum += n.radiance; ++count; } }
