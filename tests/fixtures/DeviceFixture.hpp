...
namespace engine::test {
  ...
  protected:
    ...
    static Window& window() { return *sharedWindow; }
    static Device& device() { return *sharedDevice; }

    // Raw pointer accessors (for APIs that need pointers)
    static Window* windowPtr() { return sharedWindow.get(); }
    static Device* devicePtr() { return sharedDevice.get(); }
