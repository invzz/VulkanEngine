...
namespace engine::test {
  ...
  protected:
    ...
    // Path accessors
    const std::filesystem::path& gltfAssetsPath() const { return gltfAssetsPath_; }
    const std::filesystem::path& tempDir() const { return tempDir_; }
    const std::filesystem::path& projectRoot() const { return assetsPath_; }

    // Helper to check if a glTF model exists
    bool gltfModelExists(const std::string& modelName) const { return std::filesystem::exists(gltfAssetsPath_ / modelName / "glTF" / (modelName + ".gltf")); }

    // Helper to get glTF model path
    std::filesystem::path getGltfModelPath(const std::string& modelName) const { return gltfAssetsPath_ / modelName / "glTF" / (modelName + ".gltf"); }

    // Helper to get GLB model path
    std::filesystem::path getGlbModelPath(const std::string& modelName) const { return gltfAssetsPath_ / modelName / "glTF-Binary" / (modelName + ".glb"); }
    ...
    std::filesystem::path createInvalidGltfFile() { return createTempFile("invalid_test.gltf", "not valid gltf content"); }
