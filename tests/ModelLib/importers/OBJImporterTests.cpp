...
TEST_F(OBJImporterTest, Load_Triangle_VerticesHavePositions) {
  ...
  for (const auto& v : builder.vertices) {
    if (glm::distance(v.position, glm::vec3(0.0f, 0.0f, 0.0f)) < 0.001f) hasOrigin = true;
    if (glm::distance(v.position, glm::vec3(1.0f, 0.0f, 0.0f)) < 0.001f) hasUnitX = true;
    if (glm::distance(v.position, glm::vec3(0.5f, 1.0f, 0.0f)) < 0.001f) hasTriangleTip = true;
  }
